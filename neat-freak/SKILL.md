...
- [ ] 第一步已强制枚举所有相关文件（记忆、CLAUDE.md、docs/）
- [ ] 第二步已用变更影响矩阵覆盖所有可能波及的文档层级
- [ ] 第三步已实际修改文件（不只是"我会怎么改"的描述）
- [ ] CLAUDE.md净涨幅≤30行（没有塞历史叙事）
- [ ] 没有相对时间遗留（"今天""最近"等已替换为绝对日期）
- [ ] 记忆索引中的每个链接指向存在的文件
- [ ] 跨项目改动时上下游两边docs都已对齐
...
**学到的教训：**
> 跨平台知识同步类 skill 架构清晰，Keep 的决策模式设计合理，保持 evolution 兼容即可。
